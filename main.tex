...
\icmlauthor{Alex Wortega}{main}
\end{icmlauthorlist}

\icmlaffiliation{main}{Independent Researcher}

\icmlcorrespondingauthor{Alex Wortega}{alex.n@whitecircle.ai}
...
\begin{table}[t]
\centering
\caption{Single-step position RMSE (px) per category on seen scenarios, compared
with copy-last baseline (computed on 30 randomly sampled scenes per type).
Linear extrapolation achieves $\approx$0.10\,px RMSE across all categories
(near-exact by construction; see text). Parse fail rate is 0.0\% for
PhysicsLM; LFM2 base model achieves 0\% parse rate. OOD results: Table~\ref{tab:ood}.}
\label{tab:category}
\small
\begin{tabular}{lrr@{\hspace{6pt}}r}
\toprule
\textbf{Category} & \textbf{PhysicsLM} & \textbf{Copy-last} & \textbf{Loss} \\
                  & \textbf{RMSE $\downarrow$} & \textbf{RMSE $\downarrow$} & \\
\midrule
Stacking    &  \textbf{0.46} &  1.21 & 0.518 \\
Constraint  &  \textbf{1.01} &  3.75 & 0.572 \\
Collision   &  \textbf{5.16} &  7.47 & 0.688 \\
Ramp        &  \textbf{6.13} &  6.20 & 0.674 \\
Complex     &        23.64   &  \textbf{4.30} & 0.844 \\
Minigame    &        42.52   &  \textbf{5.77} & 0.976 \\
\midrule
\textbf{Overall (seen)} & 18.85 & \textbf{5.22} & 0.660 \\
\bottomrule
\end{tabular}
\end{table}
...
The \emph{linear extrapolation} baseline achieves $\approx$0.10\,px RMSE because
Pymunk's position update is $p_{t+1} = p_t + v_{t+1}\Delta t$ (semi-implicit Euler):
given the velocity at frame $t$, position is almost exactly predictable without any
physics knowledge. The baseline fails only on collision frames, which represent a
small fraction of randomly sampled frame pairs. This means single-step MSE alone
is not a discriminating metric: even a model that perfectly learned Newtonian
integration would achieve near-zero single-step MSE on most frames.
