...
Measuring conservation in PhysicsLM's rollout predictions requires
autoregressive multi-step inference, which we defer to future work.
We note that single-step teacher-forced evaluation (Section~\ref{sec:main-results})
measures only local prediction error; rollout stability and energy drift
are open evaluation dimensions. Prior work on neural simulators reports
energy drift of 1--5\% per step for GNS-style models; we expect PhysicsLM's
drift to be higher for high-velocity scenarios given its lack of explicit
...
Elasticity      & 0.1--0.95  & $>$0.98 (superelastic) & Persistent bouncing \\
Gravity         & $-$981\,px/s$^2$ & 0, $-$163 (Moon), $-$2450 (Jupiter) & Trajectory curvature \\
\bottomrule
\end{tabular}
\end{table}
